...
1. 활성 객체 생성
  ...
  * 이는 엔진 내부에서 접근할 수 있다는 뜻으로 사용자는 접근할 수 없다.
2. arguments 객체 생성
3. 스코프 정보 생성
...
### 클로저의 활용(UseOfClosure.js, UseOfClosure2.js)
* 클로저는 성능적인 면과 자원적인 면에서 약간의 손해를 볼 수 있으므로 무차별적으로 사용해서는 안된다.
* setTimeout()에 지정되는 함수의 사용자 정의(SetTimeOut.js)
  + setTimeout()은 웹브라우저에서 제공하는 함수인데, 첫번째 인자로 넘겨지는 함수 실행의 스케줄링을 할 수 있다.
  + 두번재 인자인 밀리 초 단위 숫자만큼의 시간 간격으로 해당 함수를 반복해서 호출한다.
  + setTimeout()으로 자신의 코드를 호출하고 싶다면 첫 번째 인자로 해당 함수 객체의 참조를 넘겨주면 되지만, 이것으로는 실제 실행될 때 함수에 인자를 줄 수 없다.

### 클로저를 활용할 때 주의사항(PrecautionOfClosure.js)
* 클로저의 프로퍼티 값은 쓰기가 가능하므로 그 값이 여러번 호출되어 항상 변할 수 있음에 유의해야 한다.
* 하나의 클로저가 여러 함수 객체의 스코프 체인에 들어가 있는 경우도 있다.
* 루프 안에서 클로저를 활용할 때는 주의해야 한다.

## 객체 지향 프로그래밍

### 클래스, 생성자, 메서드(BasicOfJSClass.js)
* Java같은 경우 class라는 키워드를 제공하여 클래스를 만들고 같은 이름의 메서드로 생성자를 구현해낸다. 
* 하지만 자바스크립트에는 이러한 개념이 없다.
* 자바스크립트의 거의 모든것이 객체이고, 특히 함수객체로 많은 것을 구현해낸다. 
...
### 클래스의 기능을 가진 subClass 함수(SubClass.js)
* 프로토 타입을 이용한 상속과 클래스 기반의 상속에서 소개한 내용을 바탕으로 기존의 클래스와 같은 기능을 하는 자바스크립트 함수를 만들어 본다.
* 앞서 소개한 세가지를 활용해서 만들 예정이다.
  + 함수의 프로토타입 체인
  + extend 함수
  + 인스턴스를 생성할 때 생성자 호출(여기서는 생성자를 _init 함수로 정한다.)

* subClass는 상속받을 클래스에 넣을 변수 및 메서드가 남긴 객체를 인자로 받아 부모 함수를 상속받는 자식 클래스를 만든다. 여기서 부모함수는 subClass() 함수를 호출할 때 this객체를 의미한다.
* 자식클래스 확장 - 사용자가 인자로 넣은 객체를 자식 클래스에 넣어 자식 클래스를 확장한다.
* 생성자 호출 - 클래스의 인스턴스가 생성될 때, 클래스 내의 정의된 생성자가 호출되어야 한다. 물론 부모 클래스의 생성자 역시 호출되어야 한다.
